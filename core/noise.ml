...
let snoise2 x y =
  ...
  let contrib x y gi =
    ...
    if t < 0.0 then 0.0
      ...
      t *. t *. grad2 gi x y
  in
  45.23065 *. (contrib x0 y0 gi0 +. contrib x1 y1 gi1 +. contrib x2 y2 gi2)
...
let fractal
    ...
    coords =
  let fractal1 x =
    let rec loop acc weight i =
      if i = 0 then acc /. weight
      else
        let freq = frequency *. Float.pow lacunarity (float_of_int (i - 1)) in
        let amp = amplitude *. Float.pow persistence (float_of_int (i - 1)) in
        loop (acc +. amp *. snoise1 (x *. freq)) (weight +. amp) (i - 1)
    in
  ...
  let fractal2 x y =
    let rec loop acc weight i =
      if i = 0 then acc /. weight
        ...
        let freq = frequency *. Float.pow lacunarity (float_of_int (i - 1)) in
        let amp = amplitude *. Float.pow persistence (float_of_int (i - 1)) in
        loop (acc +. amp *. snoise2 (x *. freq) (y *. freq)) (weight +. amp) (i - 1)
    in
    loop 0.0 0.0 octaves
  in
  match coords with
