...
(* Point utils *)
let splat n : Joy.point = { x = n; y = n }

let pmap2 f ({ x = x1; y = y1 } : Joy.point) ({ x = x2; y = y2 } : Joy.point) : Joy.point =
  { x = f x1 x2; y = f y1 y2 }

let ( +~ ) (p1 : Joy.point) (p2 : Joy.point) : Joy.point =
  { x = p1.x +. p2.x; y = p1.y +. p2.y }

let ( /! ) ({ x; y } : Joy.point) scalar : Joy.point =
  { x = x /. scalar; y = y /. scalar }

(* Random utils for creating random clustered points *)
let rand_point () : Joy.point =
  { x = Random.float size -. half_size; y = Random.float size -. half_size }

(* Creates a point within 50 units of a center *)
let centered_point (center : Joy.point) _ : Joy.point =
...
type box = { min : Joy.point; max : Joy.point }
...
let contains { min; max } ({ x; y } : Joy.point) =
