...
let palette =
  [
    (* purple *)      rgb 107 108 163;
    (* light blue *)  rgb 135 188 189;
    (* green *)       rgb 111 153 84;
    (* light purple *)rgb 150 155 199;
    (* light green *) rgb 137 171 124;
    (* dark purple *)  rgb 67 68 117;
    (* darker purple *)rgb 44 45 84;
  ]

(* utility functions *)
let rand_nth coll = List.length coll |> Random.int |> List.nth coll
...
(* Creates a list of packed (non-overlapping) circles *)
let pack_circles () =
  ...
  in
  (* attempts to create a circle that meets all the necessary conditions,
