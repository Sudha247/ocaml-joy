...
let shuffle xs =
  let pairs = List.map (fun c -> (Random.int 0xFFFF, c)) xs in
...
let grid division =
  let grid_size = size / division in
...
let make_line flowfield (x, y) =
  ...
  (line ~a:{ x = ax; y = ay } { x = bx; y = by }, (cx, cy))
